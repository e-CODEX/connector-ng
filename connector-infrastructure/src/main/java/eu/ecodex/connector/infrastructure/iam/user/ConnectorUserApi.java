 ...
 */
@RequestMapping(path = "/api/v1/users", consumes = MediaType.APPLICATION_JSON_VALUE)
...
public interface ConnectorUserApi {
    ...
    ConnectorUserDto register(@Valid @RequestBody ConnectorUserDto userDto);

    @ResponseStatus(HttpStatus.ACCEPTED)
    @Operation(summary = "Update a connector user.")
    @PutMapping("/{id}")
    @ApiResponses(@ApiResponse(responseCode = "400", description = "Bad Request"))
    ConnectorUserDto update(@PathVariable("id") Long id,
                            @Valid @RequestBody ConnectorUserDto userDto);

    @ResponseStatus(HttpStatus.ACCEPTED)
    @Operation(summary = "Update partially a connector user.")
    @PatchMapping("/{id}")
    @ApiResponses(@ApiResponse(responseCode = "400", description = "Bad Request"))
    ConnectorUserDto patch(@PathVariable("id") Long id,
                           @Valid @RequestBody ConnectorUserDto userDto);
    ...
    ConnectorUserDto getById(@PathVariable("id") Long identifier);
    ...
    @Operation(summary = "Delete a connector user.")
    @DeleteMapping
    @ApiResponses(@ApiResponse(responseCode = "404", description = "Not Found"))
    void delete(@RequestBody ConnectorUserDto userDto);
