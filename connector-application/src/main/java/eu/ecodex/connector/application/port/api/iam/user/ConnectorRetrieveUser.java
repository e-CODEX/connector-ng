 ...
 * Provides methods to retrieve users by their unique identifier, username, email, or a combination of username and email.
 */
public interface ConnectorRetrieveUser {
     ...
     * @throws ConnectorUserNotFoundException if no user is found with the given username and email address
     */
    ConnectorUser getByUsernameAndEmail(String username, String email) throws
            ConnectorUserNotFoundException;

    /**
     * Retrieves an {@link Optional} containing a {@link ConnectorUser} instance associated with the given username.
     * If no user is found with the specified username, an empty {@link Optional} is returned.
     *
     * @param username the unique username of the user to retrieve
     * @return an {@link Optional} containing the {@link ConnectorUser} if found, or an empty {@link Optional} if not
     */
